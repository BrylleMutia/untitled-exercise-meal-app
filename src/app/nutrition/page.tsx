    ...
    <div className="grid min-w-0 gap-4 pb-4">
        ...
        <Card>
          ...
          <ul className="mt-2 grid gap-1.5">
            {todaysPlan.map((pm) => (
              <li
                key={pm.id}
                className={`flex items-center justify-between rounded-xl bg-cream px-3 py-2 text-sm font-semibold ${
                  pm.skipped ? "opacity-50" : ""
                }`}
              >
                <span className="min-w-20 font-bold text-ink-soft">{slotLabels[pm.slot]}</span>
                <span className={pm.skipped ? "line-through" : ""}>{pm.label}</span>
                <button
                  type="button"
      ...
      <Card>
        ...
        <p className="text-xs font-semibold text-muted">
          Log a whole recipe with one tap — quantities stay adjustable later.
        </p>
        <ul className="mt-3 grid gap-1.5">
          {snapshot.savedMeals.map((meal) => (
            <li key={meal.id} className="flex items-center justify-between rounded-xl bg-cream px-3 py-2">
              <div>
